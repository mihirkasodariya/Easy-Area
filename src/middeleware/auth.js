...
const validateAccessToken = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ success: false, status: 401, message: "No token provided!" });
    }
    const secret = process.env.JWT_SECRET;
    const verifyOptions = {
      issuer: "tracking",
    ...
    if (!rootUser) {
      return res
        .status(401)
        .json({ success: false, status: 401, message: "Unauthorized User" });
    }
...
const authorizeRoles = (...allowedRoles) => {
  return (req, res, next) => {
    if (!req.user || !allowedRoles.includes(req.user.role)) {
      return res
        .status(403)
        .json({ success: false, status: 0, message: "Access Denied" });
    }
    next();
  };
};
